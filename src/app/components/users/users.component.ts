...
setTimeout(() => {
  ...
  ngOnInit() {  
    ...
    this.service.getMessages()
      .subscribe((data) => {
        ...
        if (index == -1 && !data.agentid && this.list[0].end ) {
          this.list.unshift({
            'name': data.firstName,
            'id': data.body.fbId,
            'pic': data.profilePic,
            'message':data.conversation.resData,
            'status': 'Requested',
            'end': false,
            'agentchat':[],
            'agentid':''
          ...
          if(localStorage.getItem('id')==data.body.fbId){
  ...
  this.isSelected=true;
    ...
    this.service.endchat(localStorage.getItem('id'),this.user)
    const index = this.list.findIndex(x => x.id === localStorage.getItem('id'));
    ...
    this.http.post('https://da582a5a.ngrok.io/endchat',this.data).subscribe(res => {
      console.log(res);
    });
    this.conversation = [];
    console.log(this.list);
  ...
  onSubmit() {
    ...
    const index = this.list.findIndex(x => x.id === localStorage.getItem('id'));
    this.addMessage({ from: 'agent', text: this.getMessage(), type: 'Bot', date: new Date().getTime() });
    this.service.sendMessage(this.getMessage(), localStorage.getItem('id'));
    // this.clearMessage();
    this.focusMessage();
  }
}
  public addMessage({ from, text, type, date }: { from; text; type: 'USER' | 'Bot'; date; }) {
    console.log(text)
    if(text!='')
